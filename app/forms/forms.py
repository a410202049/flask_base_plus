...
from app import db
from app.models.Models import MenuAuth,User
    ...
    def validate_method(self, field):
        if (self.id.data is not None):
            ...
            menu = db.session.query(MenuAuth)\
                ...
                .filter(MenuAuth.parent_id ==self.parent_id.data,or_(MenuAuth.method == self.method.data,MenuAuth.name==self.menu_name.data)).scalar()
            if menu is not None:
